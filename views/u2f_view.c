...
static void u2f_view_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(model->display_msg == U2fMsgNotConnected) {
        ...
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 20, AlignCenter, AlignBottom, "Ready.");
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 34, AlignCenter, AlignBottom, "Bring to");
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 48, AlignCenter, AlignBottom, "NFC reader");
    } else if(model->display_msg == U2fMsgIdle) {
        canvas_draw_icon(canvas, 0, 0, &I_NFC_dolphin_emulation_51x64);
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 34, AlignCenter, AlignBottom, "Connected!");
    } else if(model->display_msg == U2fMsgRegister) {
        canvas_draw_icon(canvas, 0, 0, &I_NFC_dolphin_emulation_51x64);
        canvas_draw_str_aligned(
            canvas, 128 / 2 + 25, 34, AlignCenter, AlignBottom, "Registering...");
    } else if(model->display_msg == U2fMsgAuth) {
        canvas_draw_icon(canvas, 0, 0, &I_NFC_dolphin_emulation_51x64);
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 27, AlignCenter, AlignBottom, "Authenti-");
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 41, AlignCenter, AlignBottom, "cating...");
    } else if(model->display_msg == U2fMsgSuccess) {
        canvas_draw_icon(canvas, 0, 9, &I_DolphinSuccess_91x55);
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(canvas, 75, 12, AlignLeft, AlignTop, "Success!");
    } else if(model->display_msg == U2fMsgError) {
        canvas_draw_icon(canvas, 0, 0, &I_NFC_dolphin_emulation_51x64);
        canvas_draw_str_aligned(canvas, 128 / 2 + 25, 34, AlignCenter, AlignBottom, "Cert error!");
    }
}
