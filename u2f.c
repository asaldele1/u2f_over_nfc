...
U2fData* u2f_alloc(void) {
    U2fData* u2f = malloc(sizeof(U2fData));
    if(u2f) {
        memset(u2f, 0, sizeof(U2fData));
    }
    return u2f;
}
...
static uint16_t u2f_authenticate(U2fData* U2F, uint8_t* buf) {
    ...
    const U2fAuthReq* req = (const U2fAuthReq*)buf;
    const uint8_t* key_handle_data = buf + sizeof(U2fAuthReq);
    uint8_t key_handle_len = key_handle_data[0];
    const uint8_t* key_handle = key_handle_data + 1;
    ...
    if(key_handle_len != U2F_EC_BIGNUM_SIZE * 2) {
        FURI_LOG_W(TAG, "Wrong key handle length: %u", key_handle_len);
...

    {
        ...
        // Recover private key
        MCHECK(mbedtls_md_hmac_update(&hmac_ctx, req->app_id, sizeof(req->app_id)));
        MCHECK(mbedtls_md_hmac_update(&hmac_ctx, key_handle + U2F_HASH_SIZE, U2F_HASH_SIZE));
...
uint16_t u2f_applet_selection(uint8_t* buf) {
    if(buf[4] != 8 || memcmp(rid_ac_ax, &buf[5], 8) != 0) {
...
uint16_t u2f_msg_parse(U2fData* U2F, uint8_t* buf, uint16_t len) {
    ...
    if(buf[1] == U2F_CMD_REGISTER || buf[1] == U2F_CMD_AUTHENTICATE) {
        if(len < 5 + U2F_CHALLENGE_SIZE + U2F_APP_ID_SIZE) {
            memcpy(&buf[0], state_wrong_data, 2);
            return 2;
        }
    ...
    } else if(buf[1] == U2F_CMD_APPLET_SELECTION) {
        if(len < 5 || len < 5 + buf[4]) {
            memcpy(&buf[0], state_wrong_data, 2);
            return 2;
        }
